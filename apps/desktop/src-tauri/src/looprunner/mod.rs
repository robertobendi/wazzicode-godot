...
const BUILDER_TAIL: &str = "\n\nAfter making changes, run godot_verify. It must prove headless import and GDScript syntax; never imply that it ran tests. If res://tests/run_tests.gd exists, also run godot_test_run and address its independent pass, fail, or timeout result. When runtime behavior matters, use godot_debug_run and address its bounded diagnostic and performance evidence. Inspect the relevant scene tree and capture the 2D or 3D editor viewport when additional visual evidence is useful.\n\nEND your reply with EXACTLY one fenced json block and NOTHING after it:\n```json\n{\"status\":\"done|continue|blocked\",\"summary\":\"<one sentence>\",\"screenshotPath\":\"<returned screenshot path, or empty>\"}\n```\nUse \"done\" ONLY when the WHOLE goal is achieved, godot_verify passes, and any configured project test runner passes. Use \"continue\" when there is more to do. Use \"blocked\" only if you genuinely cannot make progress.";
...
fn qa_prompt(goal: &str, images: &[String]) -> String {
    let refs = reference_block(images);
    format!(
        "You are a harsh, skeptical QA reviewer. Judge whether this goal has been FULLY achieved in the CURRENT Godot project.\n\nGOAL:\n{goal}\n\nReference images:\n{refs}\n\nJudge in this order:\n\n1. Run `godot_verify` FIRST. It is the ground truth for headless import and GDScript syntax. Report `gate` as \"pass\" only when it passes, \"fail\" when it fails, or \"unavailable\" when it cannot run. Never present this gate as test execution. If res://tests/run_tests.gd exists, also run `godot_test_run`; a configured test failure or timeout blocks approval.\n2. Inspect open scenes and the relevant scene tree. Use `godot_capture_2d_view` or `godot_capture_3d_view` for visual goals. When runtime behavior matters, call `godot_debug_run` and judge its grouped diagnostics, performance findings, lifecycle, and game-frame evidence. Treat a clean result only as a clean bounded observation.\n\nSet `pass` to true only when godot_verify passed, any configured project runner passed, AND the goal is fully met. Visual polish cannot override a failing gate or test suite. Be strict.\n\nEND your reply with EXACTLY one fenced json block and NOTHING after it:\n```json\n{{\"pass\":true|false,\"gate\":\"pass|fail|unavailable\",\"score\":0,\"notes\":\"<what is wrong, what is good, and the independent project-test result when configured>\"}}\n```"
    )
}
...
mod tests {
    ...
    fn builder_prompt_switches_on_iteration() {
        ...
        assert!(first.contains("godot_test_run"));
